<template>
  ...
  >
    <div class="relative flex p-4 pb-0">
      ...
      <h3 ref="headline" class="flex-1 relative p-4 sm:ml-24">
        {{ description }}
      </h3>
    </div>
...
export default {
  ...
  mounted() {
    ...
    // svg
    this.$gsap.from(this.$refs.fingerprint, {
      drawSVG: '0% 0%',
      duration: 0.5,
      ease: 'power4.in',
      delay: 0.4,
      onComplete: () => {
        this.$gsap.to(this.$refs.fingerprint, {
    ...
    // text
    const splitted = new this.$SplitText(this.$refs.headline, {
      type: ' lines'
    })
